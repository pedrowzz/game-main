...
public abstract class SongPlayer {
    ...
    public void addPlayer(Player p) {
        synchronized (this) {
            if (!playerList.contains(p.getName())) {
                ...
                ArrayList<SongPlayer> songs = Lobby.getLobby().getPlayingSongs()
                        .get(p.getName());
                if (songs == null) {
                    songs = new ArrayList<>();
                }
                songs.add(this);
                Lobby.getLobby().getPlayingSongs().put(p.getName(), songs);
            }
        }
    }
    ...
    public void removePlayer(Player p) {
        synchronized (this) {
            ...
            if (Lobby.getLobby().getPlayingSongs().get(p.getName()) == null)
                return;

            ArrayList<SongPlayer> songs = new ArrayList<>(Lobby.getLobby().getPlayingSongs().get(p.getName()));
            songs.remove(this);
            Lobby.getLobby().getPlayingSongs().put(p.getName(), songs);
